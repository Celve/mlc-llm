...
class ModelImpl : public ModelObj {
 public:
  ...
  void CreateKVCache(KVCacheConfig kv_cache_config) final {
    if (this->kind == KVStateKind::kAttention) {
    ...
    } else if (this->kind == KVStateKind::kRNNState) {
      IntTuple max_num_sequence{kv_cache_config->max_num_sequence};
      IntTuple max_history_size = {std::max(kv_cache_config->max_history_size, 1)};
      kv_cache_ = ft_.create_kv_cache_func_(max_num_sequence, max_history_size);
      local_kv_cache_ =
          ft_.use_disco ? Downcast<DRef>(kv_cache_)->DebugGetFromRemote(0) : kv_cache_;
    }
  }
